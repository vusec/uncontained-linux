...
static int ext4_mb_init_cache(struct page *page, char *incore, gfp_t gfp)
{
	...
	if (groups_per_page > 1) {
		...
		bh = kzalloc(i, gfp);
		{
			struct buffer_head *__uncontained_tmp64;
...
int ext4_mb_alloc_groupinfo(struct super_block *sb, ext4_group_t ngroups)
{
	...
	{
		typeof((*sbi->s_group_info)) __uncontained_tmp68;
			  ...
			  struct ext4_group_desc *desc)
{
	...
	if (group % EXT4_DESC_PER_BLOCK(sb) == 0) {
		...
		{
			typeof((*meta_group_info)) __uncontained_tmp65;
...
int ext4_mb_init(struct super_block *sb)
{
	...
	{
		typeof((*sbi->s_mb_offsets)) __uncontained_tmp66;
	...
	{
		typeof((*sbi->s_mb_maxs)) __uncontained_tmp67;
