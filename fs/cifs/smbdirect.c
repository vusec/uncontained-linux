...
static int allocate_caches_and_workqueue(struct smbd_connection *info)
{
	...
	{
		struct smbd_data_transfer __uncontained_tmp114;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp114;
	}
	{
		struct smbd_request __uncontained_tmp115;
	...
	info->response_cache =
		kmem_cache_create(
			...
			0, SLAB_HWCACHE_ALIGN, NULL);
	{
		struct smbd_response __uncontained_tmp116;
...
static int allocate_mr_list(struct smbd_connection *info)
{
	...
	for (i = 0; i < info->responder_resources * 2; i++) {
		...
		smbdirect_mr->sgl = kcalloc(
					...
					GFP_KERNEL);
		{
			struct scatterlist __uncontained_tmp284;
