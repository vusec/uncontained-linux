...
int init_acl_state(struct posix_acl_state *state, int cnt)
{
	...
	{
		struct posix_ace_state_array __uncontained_tmp98;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp98;
	}
	{
		struct posix_user_ace_state __uncontained_tmp99;
		     ...
		     unsigned int uid, unsigned int gid)
{
	...
	{
		struct smb_ace __uncontained_tmp129;
...
pass:
	...
	if (nt_size > 0) {
		...
		pntsd = kzalloc(sizeof(struct smb_ntsd) + powner_sid_size +
				...
				nt_size, GFP_KERNEL);
		{
			struct smb_acl __uncontained_tmp130;
			__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp130;
		}
		{
			struct smb_ntsd __uncontained_tmp131;
