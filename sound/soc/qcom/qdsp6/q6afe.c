			   ...
			   int token)
{
	...
	{
		typeof((*param)) __uncontained_tmp111;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp111;
	}
	{
		typeof((*pdata)) __uncontained_tmp112;
				   ...
				   int param_id, int module_id, int psize)
{
	...
	p = kzalloc(pkt_size, GFP_KERNEL);
	{
		typeof((*param)) __uncontained_tmp113;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp113;
	}
	{
		typeof((*pdata)) __uncontained_tmp114;
...
int q6afe_port_stop(struct q6afe_port *port)
{
	...
	p = kzalloc(pkt_size, GFP_KERNEL);
	{
		typeof((*stop)) __uncontained_tmp115;
...
int q6afe_port_start(struct q6afe_port *port)
{
	...
	p = kzalloc(pkt_size, GFP_KERNEL);
	{
		typeof((*start)) __uncontained_tmp116;
			       ...
			       uint32_t client_handle)
{
	...
	p = kzalloc(pkt_size, GFP_KERNEL);
	{
		typeof((*vote_cfg)) __uncontained_tmp117;
