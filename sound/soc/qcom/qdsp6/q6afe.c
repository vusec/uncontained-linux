			   ...
			   int token)
{
	...
	{
		typeof((*param)) __uncontained_tmp120;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp120;
	}
	{
		typeof((*pdata)) __uncontained_tmp121;
				   ...
				   int param_id, int module_id, int psize)
{
	...
	p = kzalloc(pkt_size, GFP_KERNEL);
	{
		typeof((*param)) __uncontained_tmp122;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp122;
	}
	{
		typeof((*pdata)) __uncontained_tmp123;
...
int q6afe_port_stop(struct q6afe_port *port)
{
	...
	p = kzalloc(pkt_size, GFP_KERNEL);
	{
		typeof((*stop)) __uncontained_tmp124;
...
int q6afe_port_start(struct q6afe_port *port)
{
	...
	p = kzalloc(pkt_size, GFP_KERNEL);
	{
		typeof((*start)) __uncontained_tmp125;
			       ...
			       uint32_t client_handle)
{
	...
	p = kzalloc(pkt_size, GFP_KERNEL);
	{
		typeof((*vote_cfg)) __uncontained_tmp126;
			     ...
			     const char *client_name, uint32_t *client_handle)
{
	...
	p = kzalloc(pkt_size, GFP_KERNEL);
	{
		typeof((*vote_cfg)) __uncontained_tmp127;
