...
static int smscore_init_ir(struct smscore_device_t *coredev)
{
	...
	if (ir_io) {/* only if IR port exist we use IR sub-module */
		...
		if	(rc != 0)
			...
			{
				struct sms_msg_data2 __uncontained_tmp54;
					   ...
					   loadfirmware_t loadfirmware_handler)
{
	...
	{
		struct sms_firmware __uncontained_tmp55;
...
static int smscore_init_device(struct smscore_device_t *coredev, int mode)
{
	...
	{
		struct sms_msg_data __uncontained_tmp56;
...
int smscore_set_device_mode(struct smscore_device_t *coredev, int mode)
{
	...
	if (rc >= 0) {
		...
		buffer = kmalloc(sizeof(struct sms_msg_data) +
				 SMS_DMA_ALIGNMENT, GFP_KERNEL | coredev->gfp_buf_flags);
		{
			struct sms_msg_data __uncontained_tmp57;
