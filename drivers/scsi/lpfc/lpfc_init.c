...
lpfc_hb_eq_delay_work(struct work_struct *work)
{
	...
	ena_delay = kcalloc(phba->sli4_hba.num_possible_cpu, sizeof(*ena_delay),
			    GFP_KERNEL);
	{
		typeof((*ena_delay)) __uncontained_tmp274;
	...
	if (lpfc_is_vmid_enabled(phba)) {
		vport->vmid =
		    kcalloc(phba->cfg_max_vmid, sizeof(struct lpfc_vmid),
			    GFP_KERNEL);
		{
			struct lpfc_vmid __uncontained_tmp264;
...
lpfc_sli_driver_resource_setup(struct lpfc_hba *phba)
{
	...
	if (!phba->sli.sli3_ring) {
		phba->sli.sli3_ring = kcalloc(LPFC_SLI3_MAX_RING,
					      ...
					      GFP_KERNEL);
		{
			struct lpfc_sli_ring __uncontained_tmp265;
...
lpfc_sli4_driver_resource_setup(struct lpfc_hba *phba)
{
	...
	phba->fcf.fcf_rr_bmask = kcalloc(longs, sizeof(unsigned long),
					 GFP_KERNEL);
	{
		unsigned long __uncontained_tmp266;
	...
	phba->sli4_hba.hba_eq_hdl = kcalloc(phba->cfg_irq_chann,
					    ...
					    GFP_KERNEL);
	{
		struct lpfc_hba_eq_hdl __uncontained_tmp267;
	...
	phba->sli4_hba.cpu_map = kcalloc(phba->sli4_hba.num_possible_cpu,
					...
					GFP_KERNEL);
	{
		struct lpfc_vector_map_info __uncontained_tmp268;
	...
	phba->sli4_hba.idle_stat = kcalloc(phba->sli4_hba.num_possible_cpu,
					   ...
					   GFP_KERNEL);
	{
		typeof((*phba->sli4_hba.idle_stat)) __uncontained_tmp275;
...
lpfc_sli4_queue_create(struct lpfc_hba *phba)
{
	...
	if (!phba->sli4_hba.hdwq) {
		phba->sli4_hba.hdwq = kcalloc(
			...
			GFP_KERNEL);
		{
			struct lpfc_sli4_hdw_queue __uncontained_tmp269;
	...
	if (phba->cfg_enable_fc4_type & LPFC_ENABLE_NVME) {
		if (phba->nvmet_support) {
			phba->sli4_hba.nvmet_cqset = kcalloc(
					...
					GFP_KERNEL);
			{
				struct lpfc_queue *__uncontained_tmp270;
			...
			phba->sli4_hba.nvmet_mrq_hdr = kcalloc(
					...
					GFP_KERNEL);
			{
				struct lpfc_queue *__uncontained_tmp271;
			...
			phba->sli4_hba.nvmet_mrq_data = kcalloc(
					...
					GFP_KERNEL);
			{
				struct lpfc_queue *__uncontained_tmp272;
...
lpfc_sli4_queue_setup(struct lpfc_hba *phba)
{
	...
	if (phba->sli4_hba.cq_max) {
		...
		{
			struct lpfc_queue *__uncontained_tmp273;
