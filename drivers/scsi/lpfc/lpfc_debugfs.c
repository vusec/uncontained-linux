			      ...
			      size_t nbytes, loff_t *ppos)
{
	...
	phba->nvmeio_trc = kzalloc((sizeof(struct lpfc_debugfs_nvmeio_trc) *
				    sz), GFP_KERNEL);
	{
		struct lpfc_debugfs_nvmeio_trc __uncontained_tmp39;
	...
	if (!phba->hba_debugfs_root) {
		...
		if (!phba->slow_ring_trc) {
			phba->slow_ring_trc = kmalloc(
				...
				GFP_KERNEL);
			{
				struct lpfc_debugfs_trc __uncontained_tmp38;
		...
		if (lpfc_debugfs_max_nvmeio_trc) {
			...
			{
				struct lpfc_debugfs_nvmeio_trc __uncontained_tmp40;
...
nvmeio_off:
	...
	vport->disc_trc = kzalloc(
		...
		GFP_KERNEL);
	{
		struct lpfc_debugfs_trc __uncontained_tmp41;
