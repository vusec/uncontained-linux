...
static int beiscsi_alloc_mem(struct beiscsi_hba *phba)
{
	...
	phwi_ctrlr->wrb_context = kcalloc(phba->params.cxns_per_ctrl,
					  ...
					  GFP_KERNEL);
	{
		struct hwi_wrb_context __uncontained_tmp121;
	...
	phba->init_mem = kcalloc(SE_MEM_MAX, sizeof(*mem_descr),
				 GFP_KERNEL);
	{
		typeof((*mem_descr)) __uncontained_tmp129;
...
static int beiscsi_init_wrb_handle(struct beiscsi_hba *phba)
{
	...
	phwi_ctxt->be_wrbq = kcalloc(phba->params.cxns_per_ctrl,
				     ...
				     GFP_KERNEL);
	{
		struct be_queue_info __uncontained_tmp122;
	...
	for (index = 0; index < phba->params.cxns_per_ctrl; index++) {
		...
		pwrb_context->pwrb_handle_base =
				kcalloc(phba->params.wrbs_per_cxn,
					...
					GFP_KERNEL);
		{
			struct wrb_handle *__uncontained_tmp123;
		...
		pwrb_context->pwrb_handle_basestd =
				kcalloc(phba->params.wrbs_per_cxn,
					...
					GFP_KERNEL);
		{
			struct wrb_handle *__uncontained_tmp124;
...
static int beiscsi_init_sgl_handle(struct beiscsi_hba *phba)
{
	...
	if (1 == mem_descr_sglh->num_elements) {
		phba->io_sgl_hndl_base = kcalloc(phba->params.ios_per_ctrl,
						 ...
						 GFP_KERNEL);
		{
			struct sgl_handle *__uncontained_tmp125;
		...
		{
			struct sgl_handle *__uncontained_tmp126;
...
static int hba_setup_cid_tbls(struct beiscsi_hba *phba)
{
		...
		if (test_bit(ulp_num, (void *)&phba->fw_config.ulp_supported)) {
			...
			ptr_cid_info->cid_array =
				kcalloc(BEISCSI_GET_CID_COUNT(phba, ulp_num),
					...
					GFP_KERNEL);
			{
				typeof((*ptr_cid_info->cid_array)) __uncontained_tmp130;
	...
	phba->ep_array = kcalloc(phba->params.cxns_per_ctrl,
				 ...
				 GFP_KERNEL);
	{
		struct iscsi_endpoint *__uncontained_tmp127;
	...
	phba->conn_table = kcalloc(phba->params.cxns_per_ctrl,
				   ...
				   GFP_KERNEL);
	{
		struct beiscsi_conn *__uncontained_tmp128;
