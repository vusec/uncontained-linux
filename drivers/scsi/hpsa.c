...
static int hpsa_allocate_ioaccel2_sg_chain_blocks(struct ctlr_info *h)
{
	...
	h->ioaccel2_cmd_sg_list =
		kcalloc(h->nr_cmds, sizeof(*h->ioaccel2_cmd_sg_list),
					GFP_KERNEL);
	{
		typeof((*h->ioaccel2_cmd_sg_list)) __uncontained_tmp130;
...
static int hpsa_alloc_sg_chain_blocks(struct ctlr_info *h)
{
	...
	h->cmd_sg_list = kcalloc(h->nr_cmds, sizeof(*h->cmd_sg_list),
				 GFP_KERNEL);
	{
		typeof((*h->cmd_sg_list)) __uncontained_tmp131;
...
static void hpsa_update_scsi_devices(struct ctlr_info *h)
{
	...
	{
		typeof((*currentsd)) __uncontained_tmp132;
...
static int hpsa_alloc_cmd_pool(struct ctlr_info *h)
{
	h->cmd_pool_bits = kcalloc(DIV_ROUND_UP(h->nr_cmds, BITS_PER_LONG),
				   ...
				   GFP_KERNEL);
	{
		unsigned long __uncontained_tmp127;
...
static struct ctlr_info *hpda_alloc_ctlr_info(void)
{
	...
	{
		typeof((*h->reply_map)) __uncontained_tmp133;
