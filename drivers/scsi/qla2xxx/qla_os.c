				...
				struct rsp_que *rsp)
{
	...
	ha->req_q_map = kcalloc(ha->max_req_queues, sizeof(struct req_que *),
				GFP_KERNEL);
	{
		struct req_que *__uncontained_tmp262;
	...
	ha->rsp_q_map = kcalloc(ha->max_rsp_queues, sizeof(struct rsp_que *),
				GFP_KERNEL);
	{
		struct rsp_que *__uncontained_tmp263;
	...
	if ((ql2xmqsupport || ql2xnvmeenable) && ha->max_qpairs) {
		ha->queue_pair_map = kcalloc(ha->max_qpairs, sizeof(struct qla_qpair *),
			GFP_KERNEL);
		{
			struct qla_qpair *__uncontained_tmp264;
	...
	*rsp = kzalloc(sizeof(struct rsp_que), GFP_KERNEL);
	...
	if (ha->nvram_npiv_size) {
		ha->npiv_info = kcalloc(ha->nvram_npiv_size,
					...
					GFP_KERNEL);
		{
			struct qla_npiv_entry __uncontained_tmp265;
