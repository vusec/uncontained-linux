...
static int backend_unmap(struct xen_front_pgdir_shbuf *buf)
{
	...
	unmap_ops = kcalloc(buf->num_pages, sizeof(*unmap_ops),
			    GFP_KERNEL);
	{
		typeof((*unmap_ops)) __uncontained_tmp133;
...
static int backend_map(struct xen_front_pgdir_shbuf *buf)
{
	...
	{
		typeof((*map_ops)) __uncontained_tmp134;
	...
	buf->backend_map_handles = kcalloc(buf->num_pages,
					   ...
					   GFP_KERNEL);
	{
		typeof((*buf->backend_map_handles)) __uncontained_tmp135;
...
static int alloc_storage(struct xen_front_pgdir_shbuf *buf)
{
	...
	{
		typeof((*buf->grefs)) __uncontained_tmp136;
