...
il_init_channel_map(struct il_priv *il)
{
	...
	il->channel_info =
	    kcalloc(il->channel_count, sizeof(struct il_channel_info),
		    GFP_KERNEL);
	{
		struct il_channel_info __uncontained_tmp61;
...
il_init_geos(struct il_priv *il)
{
	...
	channels =
	    kcalloc(il->channel_count, sizeof(struct ieee80211_channel),
		    GFP_KERNEL);
	{
		struct ieee80211_channel __uncontained_tmp65;
	...
	rates =
	    kzalloc((sizeof(struct ieee80211_rate) * RATE_COUNT_LEGACY),
		    GFP_KERNEL);
	{
		struct ieee80211_rate __uncontained_tmp32;
	...
	if (!il->txq) {
		il->txq =
		    kcalloc(il->cfg->num_of_queues,
			    ...
			    GFP_KERNEL);
		{
			struct il_tx_queue __uncontained_tmp66;
