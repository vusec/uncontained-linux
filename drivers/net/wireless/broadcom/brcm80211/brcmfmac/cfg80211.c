...
static s32 brcmf_init_priv_mem(struct brcmf_cfg80211_info *cfg)
{
	...
	{
		typeof((*cfg->conf)) __uncontained_tmp45;
	...
	{
		typeof((*cfg->wowl.nd)) __uncontained_tmp46;
	...
	cfg->wowl.nd_info = kzalloc(sizeof(*cfg->wowl.nd_info) +
				    ...
				    GFP_KERNEL);
	{
		typeof((*cfg->wowl.nd_info)) __uncontained_tmp47;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp47;
	}
	{
		struct cfg80211_wowlan_nd_match *__uncontained_tmp44;
...
static int brcmf_setup_ifmodes(struct wiphy *wiphy, struct brcmf_if *ifp)
{
	...
	{
		typeof((*combo)) __uncontained_tmp129;
	...
	{
		typeof((*c0_limits)) __uncontained_tmp130;
	...
	if (p2p && !rsdb) {
		...
		{
			typeof((*p2p_limits)) __uncontained_tmp131;
	...
	if (mbss) {
		...
		mbss_limits = kcalloc(n_limits, sizeof(*mbss_limits),
				      GFP_KERNEL);
		{
			typeof((*mbss_limits)) __uncontained_tmp132;
