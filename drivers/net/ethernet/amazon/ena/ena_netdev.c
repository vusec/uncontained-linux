...
static int ena_setup_tx_resources(struct ena_adapter *adapter, int qid)
{
	...
	{
		struct ena_tx_buffer __uncontained_tmp50;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp50;
	}
	if (!tx_ring->tx_buffer_info) {
		tx_ring->tx_buffer_info = vzalloc(size);
		{
			struct ena_tx_buffer __uncontained_tmp42;
	...
	if (!tx_ring->push_buf_intermediate_buf) {
		...
		{
			struct ena_tx_buffer __uncontained_tmp44;
			__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp44;
		}
		{
			u16 __uncontained_tmp46;
				  ...
				  u32 qid)
{
	...
	{
		struct ena_rx_buffer __uncontained_tmp52;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp52;
	}
	if (!rx_ring->rx_buffer_info) {
		rx_ring->rx_buffer_info = vzalloc(size);
		{
			struct ena_rx_buffer __uncontained_tmp47;
