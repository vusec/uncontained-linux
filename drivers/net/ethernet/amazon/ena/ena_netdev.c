...
static int ena_setup_tx_resources(struct ena_adapter *adapter, int qid)
{
	...
	{
		struct ena_tx_buffer __uncontained_tmp41;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp41;
	}
	if (!tx_ring->tx_buffer_info) {
		tx_ring->tx_buffer_info = vzalloc(size);
		{
			struct ena_tx_buffer __uncontained_tmp33;
	...
	if (!tx_ring->push_buf_intermediate_buf) {
		...
		{
			struct ena_tx_buffer __uncontained_tmp35;
				  ...
				  u32 qid)
{
	...
	{
		struct ena_rx_buffer __uncontained_tmp43;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp43;
	}
	if (!rx_ring->rx_buffer_info) {
		rx_ring->rx_buffer_info = vzalloc(size);
		{
			struct ena_rx_buffer __uncontained_tmp38;
