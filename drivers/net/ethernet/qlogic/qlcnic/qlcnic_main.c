...
int qlcnic_setup_tss_rss_intr(struct qlcnic_adapter *adapter)
{
	...
	if (!adapter->msix_entries) {
		adapter->msix_entries = kcalloc(num_msix,
						...
						GFP_KERNEL);
		{
			struct msix_entry __uncontained_tmp169;
...
int qlcnic_enable_msix(struct qlcnic_adapter *adapter, u32 num_msix)
{
	...
	if (!adapter->msix_entries) {
		adapter->msix_entries = kcalloc(num_msix,
						...
						GFP_KERNEL);
		{
			struct msix_entry __uncontained_tmp170;
	...
	if (qlcnic_check_multi_tx(adapter) &&
		...
		ahw->intr_tbl =
			vzalloc(array_size(sizeof(struct qlcnic_intrpt_config),
					   ahw->num_msix));
		{
			struct qlcnic_intrpt_config __uncontained_tmp56;
...
static int qlcnic_get_act_pci_func(struct qlcnic_adapter *adapter)
{
	...
	{
		typeof((*pci_info)) __uncontained_tmp176;
...
int qlcnic_init_pci_info(struct qlcnic_adapter *adapter)
{
	...
	pci_info = kcalloc(ahw->max_vnic_func, sizeof(*pci_info), GFP_KERNEL);
	{
		typeof((*pci_info)) __uncontained_tmp177;
	...
	adapter->npars = kcalloc(act_pci_func,
				 ...
				 GFP_KERNEL);
	{
		struct qlcnic_npar_info __uncontained_tmp171;
	...
	adapter->eswitch = kcalloc(QLCNIC_NIU_MAX_XG_PORTS,
				   ...
				   GFP_KERNEL);
	{
		struct qlcnic_eswitch __uncontained_tmp172;
			  ...
			  struct net_device *netdev)
{
	...
	{
		struct qlcnic_host_tx_ring __uncontained_tmp173;
...
void qlcnic_alloc_lb_filters_mem(struct qlcnic_adapter *adapter)
{
	...
	{
		struct hlist_head __uncontained_tmp174;
	...
	head = kcalloc(adapter->rx_fhash.fbucket_size,
		       sizeof(struct hlist_head), GFP_ATOMIC);
	{
		struct hlist_head __uncontained_tmp175;
