...
static int bnx2x_alloc_fp_mem_at(struct bnx2x *bp, int index)
{
	...
	if (!skip_tx_queue(bp, index)) {
		...
		for_each_cos_in_tx_queue(fp, cos) {
			...
			txdata->tx_buf_ring = kcalloc(NUM_TX_BD,
						      ...
						      GFP_KERNEL);
			{
				struct sw_tx_bd __uncontained_tmp62;
	...
	if (!skip_rx_queue(bp, index)) {
		...
		{
			struct sw_rx_bd __uncontained_tmp63;
		...
		bnx2x_fp(bp, index, rx_page_ring) =
			kcalloc(NUM_RX_SGE, sizeof(struct sw_rx_page),
				GFP_KERNEL);
		{
			struct sw_rx_page __uncontained_tmp64;
...
int bnx2x_alloc_mem_bp(struct bnx2x *bp)
{
	...
	for (i = 0; i < bp->fp_array_size; i++) {
		...
		{
			struct bnx2x_agg_info __uncontained_tmp65;
	...
	bp->sp_objs = kcalloc(bp->fp_array_size, sizeof(struct bnx2x_sp_objs),
			      GFP_KERNEL);
	{
		struct bnx2x_sp_objs __uncontained_tmp66;
	...
	bp->fp_stats = kcalloc(bp->fp_array_size, sizeof(struct bnx2x_fp_stats),
			       GFP_KERNEL);
	{
		struct bnx2x_fp_stats __uncontained_tmp67;
	...
	bp->bnx2x_txq = kcalloc(txq_array_size, sizeof(struct bnx2x_fp_txdata),
				GFP_KERNEL);
	{
		struct bnx2x_fp_txdata __uncontained_tmp68;
	...
	{
		typeof((*tbl)) __uncontained_tmp70;
