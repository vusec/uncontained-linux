...
static int init_stats_buffers(struct ibmvnic_adapter *adapter)
{
	adapter->tx_stats_buffers =
				kcalloc(IBMVNIC_MAX_QUEUES,
					...
					GFP_KERNEL);
	{
		struct ibmvnic_tx_queue_stats __uncontained_tmp165;
	...
	adapter->rx_stats_buffers =
				kcalloc(IBMVNIC_MAX_QUEUES,
					...
					GFP_KERNEL);
	{
		struct ibmvnic_rx_queue_stats __uncontained_tmp166;
...
static int init_rx_pools(struct net_device *netdev)
{
	...
	adapter->rx_pool = kcalloc(num_pools,
				   ...
				   GFP_KERNEL);
	{
		struct ibmvnic_rx_pool __uncontained_tmp167;
	...
	for (i = 0; i < num_pools; i++) {
		...
		rx_pool->rx_buff = kcalloc(rx_pool->size,
					   ...
					   GFP_KERNEL);
		{
			struct ibmvnic_rx_buff __uncontained_tmp169;
			    ...
			    int pool_size, int buf_size)
{
	...
	tx_pool->tx_buff = kcalloc(pool_size,
				   ...
				   GFP_KERNEL);
	{
		struct ibmvnic_tx_buff __uncontained_tmp170;
...
static int init_tx_pools(struct net_device *netdev)
{
	...
	{
		struct ibmvnic_tx_pool __uncontained_tmp172;
	...
	adapter->tso_pool = kcalloc(num_pools,
				    sizeof(struct ibmvnic_tx_pool), GFP_KERNEL);
	{
		struct ibmvnic_tx_pool __uncontained_tmp173;
...
static int init_napi(struct ibmvnic_adapter *adapter)
{
	...
	{
		struct napi_struct __uncontained_tmp174;
...
static int init_sub_crqs(struct ibmvnic_adapter *adapter)
{
	...
	{
		typeof((*allqueues)) __uncontained_tmp175;
	...
	{
		typeof((*adapter->tx_scrq)) __uncontained_tmp176;
	...
	{
		typeof((*adapter->rx_scrq)) __uncontained_tmp177;
