...
int lio_setup_glists(struct octeon_device *oct, struct lio *lio, int num_iqs)
{
	...
	{
		typeof((*lio->glist_lock)) __uncontained_tmp148;
	...
	{
		typeof((*lio->glist)) __uncontained_tmp149;
	...
	lio->glists_virt_base = kcalloc(num_iqs, sizeof(*lio->glists_virt_base),
					GFP_KERNEL);
	{
		typeof((*lio->glists_virt_base)) __uncontained_tmp150;
		__uncontained_kcalloc = (unsigned long)&__uncontained_tmp150;
	}
	lio->glists_dma_base = kcalloc(num_iqs, sizeof(*lio->glists_dma_base),
				       GFP_KERNEL);
	{
		typeof((*lio->glists_dma_base)) __uncontained_tmp151;
...
int octeon_setup_interrupt(struct octeon_device *oct, u32 num_ioqs)
{
	...
	if (oct->msix_on) {
		...
		oct->msix_entries = kcalloc(oct->num_msix_irqs,
					    ...
					    GFP_KERNEL);
		{
			struct msix_entry __uncontained_tmp147;
