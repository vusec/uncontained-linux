...
mlxsw_sp_fib_entry_priv_create(const struct mlxsw_sp_router_ll_ops *ll_ops)
{
	...
	{
		typeof((*priv)) __uncontained_tmp22;
...
static int mlxsw_sp_lpm_init(struct mlxsw_sp *mlxsw_sp)
{
	...
	mlxsw_sp->router->lpm.trees = kcalloc(mlxsw_sp->router->lpm.tree_count,
					     ...
					     GFP_KERNEL);
	{
		struct mlxsw_sp_lpm_tree __uncontained_tmp77;
...
static int mlxsw_sp_vrs_init(struct mlxsw_sp *mlxsw_sp)
{
	...
	mlxsw_sp->router->vrs = kcalloc(max_vrs, sizeof(struct mlxsw_sp_vr),
					GFP_KERNEL);
	{
		struct mlxsw_sp_vr __uncontained_tmp78;
				...
				struct fib6_entry_notifier_info *fen6_info)
{
	...
	{
		struct fib6_info *__uncontained_tmp79;
...
static int mlxsw_sp_rifs_init(struct mlxsw_sp *mlxsw_sp)
{
	...
	mlxsw_sp->router->rifs = kcalloc(max_rifs,
					 ...
					 GFP_KERNEL);
	{
		struct mlxsw_sp_rif *__uncontained_tmp80;
...
static int mlxsw_sp_router_ll_op_ctx_init(struct mlxsw_sp_router *router)
{
	...
	router->ll_op_ctx = kzalloc(sizeof(*router->ll_op_ctx) + max_size,
				    GFP_KERNEL);
	{
		typeof((*router->ll_op_ctx)) __uncontained_tmp23;
