...
int ixgbe_setup_tx_resources(struct ixgbe_ring *tx_ring)
{
	...
	{
		struct ixgbe_tx_buffer __uncontained_tmp55;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp55;
	}
	if (!tx_ring->tx_buffer_info) {
		tx_ring->tx_buffer_info = vmalloc(size);
		{
			struct ixgbe_tx_buffer __uncontained_tmp53;
			     ...
			     struct ixgbe_ring *rx_ring)
{
	...
	{
		struct ixgbe_rx_buffer __uncontained_tmp56;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp56;
	}
	if (!rx_ring->rx_buffer_info) {
		rx_ring->rx_buffer_info = vmalloc(size);
		{
			struct ixgbe_rx_buffer __uncontained_tmp54;
