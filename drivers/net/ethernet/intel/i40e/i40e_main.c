...
int i40e_sync_vsi_filters(struct i40e_vsi *vsi)
{
	...
	if (!hlist_empty(&tmp_del_list)) {
		...
		{
			struct i40e_aqc_remove_macvlan_element_data __uncontained_tmp33;
	...
	if (!hlist_empty(&tmp_add_list)) {
		...
		{
			struct i40e_aqc_add_macvlan_element_data __uncontained_tmp34;
				 ...
				 enum i40e_admin_queue_opc list_type)
{
	...
	do {
		...
		{
			struct i40e_aqc_list_capabilities_element_resp __uncontained_tmp35;
...
static int i40e_vsi_alloc_arrays(struct i40e_vsi *vsi, bool alloc_qvectors)
{
	...
	{
		struct i40e_ring *__uncontained_tmp36;
	...
	if (alloc_qvectors) {
		...
		{
			struct i40e_q_vector *__uncontained_tmp37;
...
static int i40e_alloc_rings(struct i40e_vsi *vsi)
{
	...
	for (i = 0; i < vsi->alloc_queue_pairs; i++) {
		...
		{
			struct i40e_ring __uncontained_tmp172;
...
static int i40e_init_msix(struct i40e_pf *pf)
{
	...
	pf->msix_entries = kcalloc(v_budget, sizeof(struct msix_entry),
				   GFP_KERNEL);
	{
		struct msix_entry __uncontained_tmp173;
...
static int i40e_init_interrupt_scheme(struct i40e_pf *pf)
{
	...
	{
		struct i40e_lump_tracking __uncontained_tmp38;
...
#define I40E_FDEVICT_PCTYPE_DEFAULT 0xc03
	...
	{
		struct i40e_lump_tracking __uncontained_tmp40;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp40;
	}
	{
		u16 __uncontained_tmp41;
...
static int i40e_init_recovery_mode(struct i40e_pf *pf, struct i40e_hw *hw)
{
	...
	pf->vsi = kcalloc(pf->num_alloc_vsi, sizeof(struct i40e_vsi *),
			  GFP_KERNEL);
	{
		struct i40e_vsi *__uncontained_tmp174;
...
static int i40e_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	pf->vsi = kcalloc(pf->num_alloc_vsi, sizeof(struct i40e_vsi *),
			  GFP_KERNEL);
	{
		struct i40e_vsi *__uncontained_tmp175;
