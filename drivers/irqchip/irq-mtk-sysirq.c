				     ...
				     struct device_node *parent)
{
	...
	chip_data->intpol_words = kcalloc(nr_intpol_bases,
					  ...
					  GFP_KERNEL);
	{
		typeof((*chip_data->intpol_words)) __uncontained_tmp71;
	...
	chip_data->intpol_bases = kcalloc(nr_intpol_bases,
					  ...
					  GFP_KERNEL);
	{
		typeof((*chip_data->intpol_bases)) __uncontained_tmp72;
	...
	chip_data->intpol_idx = kcalloc(intpol_num,
					...
					GFP_KERNEL);
	{
		typeof((*chip_data->intpol_idx)) __uncontained_tmp73;
	...
	chip_data->which_word = kcalloc(intpol_num,
					...
					GFP_KERNEL);
	{
		typeof((*chip_data->which_word)) __uncontained_tmp74;
