...
nvmet_fc_alloc_ls_iodlist(struct nvmet_fc_tgtport *tgtport)
{
	...
	for (i = 0; i < NVMET_LS_CTX_COUNT; iod++, i++) {
		...
		iod->rqstbuf = kzalloc(sizeof(union nvmefc_ls_requests) +
				       ...
				       GFP_KERNEL);
		{
			union nvmefc_ls_requests __uncontained_tmp77;
			__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp77;
		}
		{
			union nvmefc_ls_responses __uncontained_tmp78;
			...
			struct nvmet_fc_target_port **portptr)
{
	...
	newrec = kzalloc((sizeof(*newrec) + template->target_priv_sz),
			 GFP_KERNEL);
	{
		typeof((*newrec)) __uncontained_tmp82;
