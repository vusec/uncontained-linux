				...
				size_t count)
{
	...
	for_each_cpu(cpu, mask) {
		...
		if (!rht_node) {
			...
			{
				struct sdma_engine *__uncontained_tmp13;
				__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp13;
			}
			{
				struct sdma_rht_map_elem __uncontained_tmp15;
			...
			if (!rht_node->map[vl]) {
				rht_node->map[vl] = kzalloc(sz, GFP_KERNEL);
				{
					struct sdma_engine *__uncontained_tmp14;
					__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp14;
				}
				{
					struct sdma_rht_map_elem __uncontained_tmp16;
...
int sdma_map_init(struct hfi1_devdata *dd, u8 port, u8 num_vls, u8 *vl_engines)
{
	...
	newmap = kzalloc(
		...
		GFP_KERNEL);
	{
		struct sdma_map_elem *__uncontained_tmp40;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp40;
	}
	{
		struct sdma_vl_map __uncontained_tmp41;
	...
	for (i = 0; i < newmap->vls; i++) {
		...
		if (i < newmap->actual_vls) {
			...
			newmap->map[i] = kzalloc(
				...
				GFP_KERNEL);
			{
				struct sdma_engine *__uncontained_tmp42;
				__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp42;
			}
			{
				struct sdma_map_elem __uncontained_tmp43;
...
int sdma_init(struct hfi1_devdata *dd, u8 port)
{
	...
	for (this_idx = 0; this_idx < num_engines; ++this_idx) {
		...
		sde->tx_ring =
			kvzalloc_node(array_size(descq_cnt,
				      ...
				      GFP_KERNEL, dd->node);
		{
			struct sdma_txreq *__uncontained_tmp39;
	...
	{
		typeof((*tmp_sdma_rht)) __uncontained_tmp44;
