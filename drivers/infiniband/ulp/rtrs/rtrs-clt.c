...
static int alloc_path_reqs(struct rtrs_clt_path *clt_path)
{
	...
	clt_path->reqs = kcalloc(clt_path->queue_depth,
				 ...
				 GFP_KERNEL);
	{
		typeof((*clt_path->reqs)) __uncontained_tmp110;
	...
	for (i = 0; i < clt_path->queue_depth; ++i) {
		...
		{
			typeof((*req->sge)) __uncontained_tmp111;
					...
					size_t con_num, u32 nr_poll_queues)
{
	...
	clt_path->s.con = kcalloc(total_con, sizeof(*clt_path->s.con),
				  GFP_KERNEL);
	{
		typeof((*clt_path->s.con)) __uncontained_tmp112;
	...
	if (con->c.cid == 0) {
		...
		if (!clt_path->rbufs) {
			clt_path->rbufs = kcalloc(queue_depth,
						  ...
						  GFP_KERNEL);
			{
				typeof((*clt_path->rbufs)) __uncontained_tmp113;
