...
static int mv_u3d_req_to_trb(struct mv_u3d_req *req)
{
	...
	if (length <= (unsigned)MV_U3D_EP_MAX_LENGTH_TRANSFER) {
		...
		{
			typeof((*trb)) __uncontained_tmp157;
		...
		{
			typeof((*trb_hw)) __uncontained_tmp158;
...
static int mv_u3d_probe(struct platform_device *dev)
{
	...
	{
		typeof((*u3d)) __uncontained_tmp60;
	...
	{
		struct mv_u3d_ep_context __uncontained_tmp59;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp59;
	}
	{
		struct mv_u3d_ep __uncontained_tmp60;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp60;
	}
	if (!u3d->eps) {
		retval = -ENOMEM;
	...
	{
		struct mv_u3d_req __uncontained_tmp59;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp59;
	}
	if (!u3d->status_req) {
		retval = -ENOMEM;
