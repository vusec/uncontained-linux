		...
		unsigned int max_packet, gfp_t mem_flags)
{
	...
	stream_info->stream_rings = kcalloc_node(
			...
			dev_to_node(dev));
	{
		struct xhci_ring *__uncontained_tmp4;
		...
		__le32 __iomem *addr, int max_caps)
{
	...
	if (port_cap->psi_count) {
		...
		{
			typeof((*port_cap->psi)) __uncontained_tmp6;
					...
					struct xhci_hub *rhub, gfp_t flags)
{
	...
	rhub->ports = kcalloc_node(rhub->num_ports, sizeof(*rhub->ports),
			flags, dev_to_node(dev));
	{
		typeof((*rhub->ports)) __uncontained_tmp7;
...
static int xhci_setup_port_arrays(struct xhci_hcd *xhci, gfp_t flags)
{
	...
	xhci->hw_ports = kcalloc_node(num_ports, sizeof(*xhci->hw_ports),
				flags, dev_to_node(dev));
	{
		typeof((*xhci->hw_ports)) __uncontained_tmp8;
	...
	xhci->rh_bw = kcalloc_node(num_ports, sizeof(*xhci->rh_bw), flags,
				   dev_to_node(dev));
	{
		typeof((*xhci->rh_bw)) __uncontained_tmp9;
	...
	xhci->ext_caps = kcalloc_node(cap_count, sizeof(*xhci->ext_caps),
				flags, dev_to_node(dev));
	{
		typeof((*xhci->ext_caps)) __uncontained_tmp10;
	...
	xhci->port_caps = kcalloc_node(cap_count, sizeof(*xhci->port_caps),
				flags, dev_to_node(dev));
	{
		typeof((*xhci->port_caps)) __uncontained_tmp11;
