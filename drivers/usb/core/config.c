    ...
    u8 inums[], u8 nalts[])
{
	...
	if (num_ep > 0) {
		...
		{
			struct usb_host_endpoint __uncontained_tmp76;
...
int usb_get_configuration(struct usb_device *dev)
{
	...
	{
		struct usb_host_config __uncontained_tmp77;
	...
	for (cfgno = 0; cfgno < ncfg; cfgno++) {
		...
		{
			struct usb_host_config __uncontained_tmp74;
			__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp74;
		}
		{
			char *__uncontained_tmp75;
