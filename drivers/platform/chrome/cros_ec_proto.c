			...
			struct cros_ec_command *msg)
{
	...
	if (msg->result == EC_RES_IN_PROGRESS) {
		...
		status_msg = kmalloc(sizeof(*status_msg) + sizeof(*status),
				     GFP_KERNEL);
		{
			typeof((*status)) __uncontained_tmp95;
			__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp95;
		}
		{
			typeof((*status_msg)) __uncontained_tmp96;
...
static int cros_ec_host_command_proto_query_v2(struct cros_ec_device *ec_dev)
{
	...
	{
		typeof((*msg)) __uncontained_tmp97;
...
int cros_ec_query_all(struct cros_ec_device *ec_dev)
{
	...
	proto_msg = kzalloc(sizeof(*proto_msg) + sizeof(*proto_info),
			    GFP_KERNEL);
	{
		typeof((*proto_info)) __uncontained_tmp101;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp101;
	}
	{
		typeof((*proto_msg)) __uncontained_tmp102;
		    ...
		    int insize)
{
	...
	{
		typeof((*msg)) __uncontained_tmp106;
