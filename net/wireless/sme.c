...
static int cfg80211_conn_scan(struct wireless_dev *wdev)
{
	...
	request = kzalloc(sizeof(*request) + sizeof(request->ssids[0]) +
			  ...
			  GFP_KERNEL);
	{
		typeof((*request)) __uncontained_tmp204;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp204;
	}
	{
		typeof((request->channels[0])) __uncontained_tmp205;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp205;
	}
	{
		typeof((request->ssids[0])) __uncontained_tmp206;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp206;
	}
	if (!request)
		return -ENOMEM;
			   ...
			   gfp_t gfp)
{
	...
	{
		typeof((*ev)) __uncontained_tmp207;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp207;
	}
	if (!ev) {
		     ...
		     gfp_t gfp)
{
	...
	{
		typeof((*ev)) __uncontained_tmp208;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp208;
	}
	if (!ev) {
		cfg80211_put_bss(wdev->wiphy, info->bss);
		return;
			   ...
			   bool locally_generated, gfp_t gfp)
{
	...
	{
		typeof((*ev)) __uncontained_tmp209;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp209;
	}
	if (!ev)
		return;
