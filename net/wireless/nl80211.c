	...
	if (!cb->args[0]) {
		...
		if (!attrbuf) {
			attrbuf = kcalloc(NUM_NL80211_ATTR, sizeof(*attrbuf),
					  GFP_KERNEL);
			{
				typeof((*attrbuf)) __uncontained_tmp203;
...
static int nl80211_channel_switch(struct sk_buff *skb, struct genl_info *info)
{
	...
	csa_attrs = kcalloc(NL80211_ATTR_MAX + 1, sizeof(*csa_attrs),
			    GFP_KERNEL);
	{
		typeof((*csa_attrs)) __uncontained_tmp204;
...
static int nl80211_dump_survey(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	{
		typeof((*attrbuf)) __uncontained_tmp205;
				 ...
				 struct netlink_callback *cb)
{
	...
	if (cb->args[0]) {
		...
		attrbuf = kcalloc(NUM_NL80211_ATTR, sizeof(*attrbuf),
				  GFP_KERNEL);
		{
			typeof((*attrbuf)) __uncontained_tmp206;
				   ...
				   struct cfg80211_wowlan *trig)
{
	...
	{
		typeof((*tb)) __uncontained_tmp207;
...
static int nl80211_set_wowlan(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (tb[NL80211_WOWLAN_TRIG_PKT_PATTERN]) {
		...
		new_triggers.patterns = kcalloc(n_patterns,
						...
						GFP_KERNEL);
		{
			typeof((new_triggers.patterns[0])) __uncontained_tmp208;
				       ...
				       struct cfg80211_coalesce_rules *new_rule)
{
	...
	new_rule->patterns = kcalloc(n_patterns, sizeof(new_rule->patterns[0]),
				     GFP_KERNEL);
	{
		typeof((new_rule->patterns[0])) __uncontained_tmp209;
...
static int nl80211_set_coalesce(struct sk_buff *skb, struct genl_info *info)
{
	...
	new_coalesce.rules = kcalloc(n_rules, sizeof(new_coalesce.rules[0]),
				     GFP_KERNEL);
	{
		typeof((new_coalesce.rules[0])) __uncontained_tmp210;
			     ...
			     bool tx)
{
	...
	{
		typeof((*func->rx_filters)) __uncontained_tmp211;
		...
		if (srf_tb[NL80211_NAN_SRF_BF]) {
			...
			func->srf_macs =
				kcalloc(n_entries, sizeof(*func->srf_macs),
					GFP_KERNEL);
			{
				typeof((*func->srf_macs)) __uncontained_tmp212;
				       ...
				       struct wireless_dev **wdev)
{
	...
	attrbuf = kcalloc(NUM_NL80211_ATTR, sizeof(*attrbuf), GFP_KERNEL);
	{
		typeof((*attrbuf)) __uncontained_tmp213;
...
static int nl80211_color_change(struct sk_buff *skb, struct genl_info *info)
{
	...
	{
		typeof((*tb)) __uncontained_tmp214;
