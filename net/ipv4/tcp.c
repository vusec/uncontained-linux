	...
	for_each_possible_cpu(cpu) {
		...
		if (!scratch) {
			scratch = kmalloc_node(sizeof(union tcp_md5sum_block) +
					       ...
					       cpu_to_node(cpu));
			{
				struct tcphdr __uncontained_tmp135;
				__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp135;
			}
			{
				union tcp_md5sum_block __uncontained_tmp136;
...
void __init tcp_init(void)
{
	...
	{
		struct inet_ehash_bucket __uncontained_tmp0;
	...
	tcp_hashinfo.bhash =
		alloc_large_system_hash("TCP bind",
					...
					64 * 1024);
	{
		struct inet_bind_hashbucket __uncontained_tmp1;
