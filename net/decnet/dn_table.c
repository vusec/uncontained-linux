...
static void dn_rehash_zone(struct dn_zone *dz)
{
	...
	{
		struct dn_fib_node *__uncontained_tmp312;
...
static struct dn_zone *dn_new_zone(struct dn_hash *table, int z)
{
	...
	{
		struct dn_fib_node *__uncontained_tmp313;
...
struct dn_fib_table *dn_fib_get_table(u32 n, int create)
{
	...
	t = kzalloc(sizeof(struct dn_fib_table) + sizeof(struct dn_hash),
		    GFP_KERNEL);
	{
		struct dn_fib_table __uncontained_tmp116;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp116;
	}
	{
		struct dn_hash __uncontained_tmp117;
