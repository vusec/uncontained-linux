								     ...
								     gfp_t gfp)
{
	...
	{
		typeof((*uctx)) __uncontained_tmp196;
		__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp196;
	}
						...
						void * const *ext_hdrs)
{
   ...
   not true.
	...
	if (sa->sadb_sa_auth) {
		...
		{
			typeof((*x->aalg)) __uncontained_tmp197;
			__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp197;
		}
		if (!x->aalg) {
			err = -ENOMEM;
			goto out;
	...
	if (sa->sadb_sa_encrypt) {
		if (hdr->sadb_msg_satype == SADB_X_SATYPE_IPCOMP) {
			...
			{
				typeof((*x->ealg)) __uncontained_tmp198;
				__uncontained_complex_alloc = (unsigned long)&__uncontained_tmp198;
			}
			if (!x->ealg) {
				err = -ENOMEM;
				goto out;
