...
static int __init rcu_torture_fwd_prog_init(void)
{
	...
	{
		typeof((*rfp)) __uncontained_tmp292;
		__uncontained_kcalloc = (unsigned long)&__uncontained_tmp292;
	}
	fwd_prog_tasks = kcalloc(fwd_progress, sizeof(*fwd_prog_tasks), GFP_KERNEL);
	{
		typeof((*fwd_prog_tasks)) __uncontained_tmp293;
...
static int rcu_torture_barrier_init(void)
{
	...
	barrier_cbs_tasks =
		kcalloc(n_barrier_cbs, sizeof(barrier_cbs_tasks[0]),
			GFP_KERNEL);
	{
		typeof((barrier_cbs_tasks[0])) __uncontained_tmp294;
		__uncontained_kcalloc = (unsigned long)&__uncontained_tmp294;
	}
	barrier_cbs_wq =
		kcalloc(n_barrier_cbs, sizeof(barrier_cbs_wq[0]), GFP_KERNEL);
	{
		typeof((barrier_cbs_wq[0])) __uncontained_tmp295;
...
rcu_torture_init(void)
{
	...
	if (nfakewriters > 0) {
		fakewriter_tasks = kcalloc(nfakewriters,
					   ...
					   GFP_KERNEL);
		{
			typeof((fakewriter_tasks[0])) __uncontained_tmp296;
	...
	reader_tasks = kcalloc(nrealreaders, sizeof(reader_tasks[0]),
			       GFP_KERNEL);
	{
		typeof((reader_tasks[0])) __uncontained_tmp297;
		__uncontained_kcalloc = (unsigned long)&__uncontained_tmp297;
	}
	rcu_torture_reader_mbchk = kcalloc(nrealreaders, sizeof(*rcu_torture_reader_mbchk),
					   GFP_KERNEL);
	{
		typeof((*rcu_torture_reader_mbchk)) __uncontained_tmp298;
	...
	if (nrealnocbers > 0) {
		...
		{
			typeof((nocb_tasks[0])) __uncontained_tmp299;
