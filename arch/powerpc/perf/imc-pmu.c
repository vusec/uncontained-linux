...
static int update_events_in_group(struct device_node *node, struct imc_pmu *pmu)
{
	...
	{
		struct imc_events __uncontained_tmp5;
...
static int init_nest_pmu_ref(void)
{
	...
	nest_imc_refc = kcalloc(num_possible_nodes(), sizeof(*nest_imc_refc),
								GFP_KERNEL);
	{
		typeof((*nest_imc_refc)) __uncontained_tmp11;
								...
								int pmu_index)
{
	...
	case IMC_DOMAIN_NEST:
		...
		if (!per_nest_pmu_arr) {
			per_nest_pmu_arr = kcalloc(get_max_nest_dev() + 1,
						...
						GFP_KERNEL);
			{
				struct imc_pmu *__uncontained_tmp7;
	...
	case IMC_DOMAIN_CORE:
		...
		pmu_ptr->mem_info = kcalloc(nr_cores, sizeof(struct imc_mem_info),
								GFP_KERNEL);
		{
			struct imc_mem_info __uncontained_tmp8;
		...
		core_imc_refc = kcalloc(nr_cores, sizeof(struct imc_pmu_ref),
								GFP_KERNEL);
		{
			struct imc_pmu_ref __uncontained_tmp9;
	...
	case IMC_DOMAIN_TRACE:
		...
		trace_imc_refc = kcalloc(nr_cores, sizeof(struct imc_pmu_ref),
								GFP_KERNEL);
		{
			struct imc_pmu_ref __uncontained_tmp10;
